...
#red 1 is original
red_box_1 = [-0.00012873642488892445, -3.546485802830518, 0.10193771737102325]
red_box_2 = [-0.00012873642488892445, -3.646485802830518, 0.10193771737102325]
yellow_box_1 = [5.234127829930278e-05, -2.896522629452369, 0.10193771737075434]
yellow_box_2 = [5.234127829930278e-05, -2.996522629452369, 0.10193771737075434]
blue_box_1 = [2.852663781157873e-05, -2.366442369892385, 0.10193771735674603]
blue_box_2 = [0.0035641475338084746, -2.4567790686479762, 0.10133405123468876]
green_box = [3.1465634627332903e-06, -1.9164279714890196, 0.10193771736888085]
    ...
    def loop(self):
        ...
        for color in colors:
            ...
            if colors[color] == "red": 
                ...
                present_color_1 = yellow_box_1
                present_color_2 = yellow_box_2
            ...
            for i in range(rounds):
                ...
                while self.step(self.timestep) != -1:
                    ...
                    while self.step(self.timestep) != -1:
                        ...
                        #print(list(map(lambda x: round(x, 1), self.gps.getValues())))
                        
                        if list(map(lambda x: round(x, 1), self.gps.getValues())) == list(map(lambda x: round(x, 1), present_color_1)) and i == True and direction_degrees > 0:
                            self.turn_angle(126)
                            #print("TURN")
                            i = False

                        elif list(map(lambda x: round(x, 1), self.gps.getValues())) == list(map(lambda x: round(x, 1), present_color_2)) and i == True and direction_degrees < 0:
                            self.turn_angle_c(126)
                            #print("TURN")
                            i = False

                        # BONUS
                        if self.box_sensor.getValue() < 1000 and b1 == True:
                            self.set_motors_velocity(0, 0, 0, 0)
                        ...
                        if self.box_sensor.getValue() < 1000:
                            self.set_motors_velocity(0, 0, 0, 0)
                            #print("Box_sensor: ", self.box_sensor.getValue())
                            self.open_gripper()
                            self.move_arm_to_position(desired_position, desired_velocity)
                    ...
                    while self.step(self.timestep) != -1:
                        ...
                        #if list(map(lambda x: round(x, 1), self.gps.getValues())) == list(map(lambda x: round(x, 1), end_point)):
                        if self.box_sensor.getValue() < 1000:
                            self.set_motors_velocity(0, 0, 0, 0)
                            ...
                            if b2 == True:
                                ...
                                while self.step(self.timestep) != -1:
                                    if self.receiver.getQueueLength() > 0:
                                        ...
                                        #print("Received message from Taker: ", message)
                                        if message == "Bonus":
                            ...
                            # in order
                            #self.move_distance_by_right_motor(0.1)
...
# Move arm to wall
#r.move_arm_to_position([0, -0.2, -1.4, -1.2, 0], desired_velocity)

r = RobotController()
r.loop()
#print(r.gps.getValues())
#r.move_arm_to_position(dish_position, desired_velocity)
# Basic timestep = 16
